...
export class AccessLevelData {
  ...
  saveAccessLevel(access_level: any) {
    return db.one(
      "INSERT INTO odonto.access_levels (level_name, description, id_empresa, acessa_todas_agendas, acessa_financeiro_paciente) VALUES ($1, $2, $3, $4, $5) returning *",
      [
        access_level.level_name,
        access_level.description,
        access_level.id_empresa,
        access_level.acessa_todas_agendas,
        access_level.acessa_financeiro_paciente,
      ],
    );
  }
  ...
  async deleteAccessLevel(access_level: any) {
    ...
    try {
      const resultado = await db.oneOrNone(
        "DELETE FROM odonto.access_levels WHERE access_level_id = $1 AND id_empresa = $2 RETURNING *",
        [Number(access_level.access_level_id), Number(access_level.id_empresa)],
      );
      return {
        status: "sucesso",
        data: resultado,
      ...
      if (err.code === "23503") {
        return {
          status: "erro",
          message:
            "Algum usuário está utilizando este nível de acesso. A exclusão só pode ser realizada se ninguém estiver utilizando!",
          error: err.detail,
        };
      }
  ...
  async updateAccessLevel(access_level: any) {
    ...
    try {
      return await db.none(
        `UPDATE odonto.access_levels SET level_name = $1, description = $2, acessa_todas_agendas = $5, acessa_financeiro_paciente = $6 WHERE access_level_id = $3 AND id_empresa = $4`,
        [
          access_level.level_name,
          access_level.description,
  ...
  async deleteAccessLevelScreen(access_level: any) {
    return db.none(
      "DELETE FROM odonto.access_levels_screen WHERE id_access_levels = $1 AND id_empresa = $2",
      [Number(access_level.access_level_id), Number(access_level.id_empresa)],
    );
  }
}
