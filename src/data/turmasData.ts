...
export class TurmasData {
  async saveTurma(turma: any) {
    return await db.one(
      "INSERT INTO gbmot.turmas (id_curso, dt_inicio, dt_fim, local, horario, status) VALUES ($1, $2, $3, $4, $5, $6) returning *",
      [
        turma.id_curso,
        turma.dt_inicio,
        turma.dt_fim,
  ...
  async updateTurma(turma: any) {
    await db.query(
      "UPDATE gbmot.turmas SET id_curso = $1, dt_inicio = $2, dt_fim = $3, local = $4, horario = $5, status = $6 WHERE id_turma = $7 returning *",
      [
        turma.id_curso,
        turma.dt_inicio,
        turma.dt_fim,
  ...
  async consolidarTurma(turma: any, dbOrTx: any) {
    await dbOrTx.query(
      "UPDATE gbmot.turmas SET status = $1 WHERE id_turma = $2 returning *",
      ["Consolidada", Number(turma.id_turma)],
    );
  }

  async concluirTurma(turma: any, dbOrTx: any) {
    await dbOrTx.query(
      "UPDATE gbmot.turmas SET status = $1 WHERE id_turma = $2 returning *",
      ["Concluido", Number(turma.id_turma)],
    );
  }

  async deleteTurma(data: any) {
    const result = await db.result(
      `DELETE FROM gbmot.turmas WHERE id_turma = $1 AND status <> 'Concluido'`,
