...
export class TurmaAlunoData {
  async saveTurmaAluno(inscricao: any) {
    try {
      return await db.one('INSERT INTO gbmot.turma_aluno (id_turma, id_aluno, status, dt_inscricao, id_curso) VALUES ($1, $2, $3, $4, $5) returning *',
        [inscricao.id_turma, inscricao.id_aluno, "pre-inscrito", new Date(), inscricao.id_curso])
    } catch (error: any) {
      console.error("Error saving inscrição:", error);

      if (error.code === '23505') {
        throw new Error("Você já está inscrito neste curso. Aguarde a confirmação e convocação");
      }
  ...
  async deleteTurmaAluno(id_turma: number, status: string, dbOrTx: any) {
    const result = await dbOrTx.result(`DELETE FROM gbmot.turma_aluno WHERE id_turma = $1 AND status = $2`, [id_turma, status])
    return {
