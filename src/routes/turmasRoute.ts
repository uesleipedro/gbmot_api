...
router.put(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    try {
      await turmasController.concluirTurma(req.body);
      res.status(201).json({ success: true });
    } catch (e: any) {
      return res.status(400).json({
        message: e.message || "Erro interno",
...
router.get(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    try {
      const response = await turmasController.getTurmasByStatus(
...
router.get(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    try {
      const response = await turmasController.getAlunos(
        Number(req.params.id_turma),
      );
      res.status(200).json(response);
    } catch (e: any) {
      next(e);
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!response.success)
        return res
          .status(400)
          .json({ error: "Não é possível deletar uma turma concluída" });
